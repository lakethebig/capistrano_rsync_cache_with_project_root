...
module Capistrano
  module Deploy
    module Strategy
      class RsyncWithRemoteCacheAndProjectRoot < Remote
        ...
        def rsync_command_for(server)
					...
					unless configuration[:project_root].blank?
						local_project_path = "#{local_cache_path}/#{configuration[:project_root]}"
					else
					...
					sshpass_phrase = ""
					ssh_option = ""
					unless configuration[:password].blank?
						sshpass_phrase = "sshpass -p #{configuration[:password]}"
						ssh_option = "-o StrictHostKeyChecking=no"
					end

					"rsync #{rsync_options} --rsh='#{sshpass_phrase} ssh -p #{ssh_port(server)} #{ssh_option}' #{local_project_path}/ #{rsync_host(server)}:#{repository_cache_path}/"
        end
